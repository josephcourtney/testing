...
A statement that must be true for a product, change, release, or operating decision to be justified.
...
The rule or source used to determine whether an observed result is acceptable. An oracle may be an explicit expected value, invariant, model, independent implementation, schema, stakeholder judgment, standard, baseline, or operational objective.
...
A **solitary unit test** replaces collaborators outside the focal object or function. A **sociable unit test** retains inexpensive, deterministic collaborators inside the chosen unit boundary.
...
A system test that traverses a complete workflow across the boundaries necessary to represent real use. The term is relative to the product boundary and does not necessarily imply every external production dependency.

### Resource or boundary classification

A label identifying material resources whose use affects isolation, cost, fidelity, or interpretation. Common examples include filesystem, process, database, network, broker, clock, random source, hardware accelerator, third-party service, and production environment.
...
The event or schedule at which evidence is collected, such as per-edit, pre-commit, pre-merge, scheduled, pre-release, post-deploy, or continuous.
...
A test that produces different pass/fail outcomes without a relevant change in the subject or declared inputs. Infrastructure failure should be distinguished from test nondeterminism where possible.
...
The usefulness of failure messages, logs, artifacts, context, and reproduction information in distinguishing subject failure from harness or environment failure.
...
A collaboration and specification style that describes behavior using domain-oriented examples, often expressed as Given–When–Then scenarios.
...
The proportion of instrumented executable lines observed during a specified test selection.

### Branch coverage

The proportion of instrumented branch outcomes observed during a specified test selection.
...
The proportion of generated mutants classified as detected under a declared mutation configuration and code cohort. Treatment of equivalent, timed-out, invalid, and uncovered mutants is part of the definition.

### Flake rate

A rate of suspected nondeterministic failures over a defined unit, population, and observation window.
