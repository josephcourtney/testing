...
from scripts import check_metrics, check_quality
...
def build_rows(
    ...
    *,
    ...
    complexity_average = sum(complexity_scores) / len(complexity_scores) if complexity_scores else 0.0
    ...
    significant_halstead = {
        ...
        for path in check_metrics.build_baseline(metrics, metrics_baseline.thresholds).halstead[field]
    }
    ...
    rows = [
        ...
        SummaryRow("Significant Halstead files", str(len(significant_halstead)), "baseline tracked"),
        SummaryRow(
            "Statement coverage",
        ...
        SummaryRow("Dead-code findings", str(len(quality.dead_code)), str(len(quality_baseline.dead_code))),
        SummaryRow(
            "Source duplication",
        ...
        SummaryRow("Pytest skips/xfails", str(len(quality.outcomes)), str(len(quality_baseline.outcomes))),
    ]
    if test_health is not None:
        scopes = ", ".join(f"{scope}={count}" for scope, count in test_health.scope_counts.items())
        rows.extend(
            [
                SummaryRow("Full-suite runtime", f"{test_health.duration_seconds:.2f}s", "current run"),
...
def read_test_health(path: Path) -> TestHealth:
    ...
    if not isinstance(data, dict) or data.get("version") != 4 or data.get("full_suite") is not True:
        raise ValueError("pytest health requires a version 4 full-suite evidence artifact")
    duration = data.get("duration_seconds")
    scopes = data.get("scope_counts")
    flaky = data.get("flaky_tests")
    requirements = data.get("requirement_coverage")
    if not isinstance(duration, (int, float)) or isinstance(duration, bool):
        raise ValueError("pytest health duration_seconds must be numeric")
    if not isinstance(scopes, dict) or not all(
        isinstance(scope, str) and isinstance(count, int) and not isinstance(count, bool)
        for scope, count in scopes.items()
    ):
        raise ValueError("pytest health scope_counts must map strings to integers")
    if not isinstance(flaky, list) or not all(isinstance(nodeid, str) for nodeid in flaky):
        raise ValueError("pytest health flaky_tests must be an array of strings")
    if not isinstance(requirements, dict) or not all(isinstance(value, dict) for value in requirements.values()):
        raise ValueError("pytest health requirement_coverage must be an object")
    complete = sum(value.get("complete") is True for value in requirements.values())
    return TestHealth(float(duration), dict(scopes), tuple(flaky), complete, len(requirements))
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        quality_errors = check_quality.check_snapshot(
            quality, quality_baseline, mutation=mutation
        )
        ...
        print()
        print(f"Regression gate: {'PASS' if not gate_errors else f'FAIL ({len(gate_errors)} findings)'}")
        ratchet_label = "CURRENT" if not ratchet_errors else f"STALE ({len(ratchet_errors)} findings)"
