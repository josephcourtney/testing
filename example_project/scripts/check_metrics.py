...
def collect_snapshot(source_dir: Path, root: Path) -> Snapshot:
    ...
    for file_path in sorted(source_dir.rglob("*.py")):
        ...
        for block, key in zip(blocks, block_keys, strict=True):
            if key in duplicate_keys:
                key = f"{key}@{block.lineno}"
        ...
        halstead[path] = {field: float(getattr(halstead_report, field)) for field in HALSTEAD_FIELDS}
...
def build_baseline(snapshot: Snapshot, thresholds: Thresholds) -> Baseline:
    cyclomatic = {
        key: score for key, score in snapshot.cyclomatic.items() if score >= thresholds.cyclomatic_ratchet
    }
    raw_sloc = {
        path: report.sloc
    ...
    for field in RATCHET_HALSTEAD_FIELDS:
        ...
        halstead[field] = {
            path: metrics[field] for path, metrics in snapshot.halstead.items() if metrics[field] >= threshold
        }
...
def print_report(snapshot: Snapshot, thresholds: Thresholds) -> None:
    ...
    for path, score in sorted(snapshot.maintainability.items(), key=lambda item: (item[1], item[0])):
        if score < thresholds.maintainability_ratchet:
...
def _baseline_json(baseline: Baseline) -> dict[str, JsonValue]:
    return {
        "version": 1,
        "thresholds": cast(dict[str, JsonValue], asdict(baseline.thresholds)),
        "cyclomatic": cast(dict[str, JsonValue], baseline.cyclomatic),
        "raw_sloc": cast(dict[str, JsonValue], baseline.raw_sloc),
        "maintainability": cast(dict[str, JsonValue], baseline.maintainability),
        "halstead": cast(dict[str, JsonValue], baseline.halstead),
    }
...
def _number_dict(value: JsonValue, label: str) -> dict[str, float]:
...
def read_baseline(path: Path) -> Baseline:
    ...
    if not isinstance(data, dict) or data.get("version") != 1:
        raise ValueError("metrics baseline must have version 1")

    threshold_values = _number_dict(data.get("thresholds"), "thresholds")
    defaults = Thresholds()
    thresholds = Thresholds(
        cyclomatic_ratchet=int(threshold_values["cyclomatic_ratchet"]),
        cyclomatic_ceiling=int(threshold_values["cyclomatic_ceiling"]),
        cyclomatic_tolerance=int(
            threshold_values.get("cyclomatic_tolerance", defaults.cyclomatic_tolerance)
        ),
        raw_sloc_ratchet=int(threshold_values["raw_sloc_ratchet"]),
        raw_sloc_ceiling=int(threshold_values["raw_sloc_ceiling"]),
        raw_sloc_tolerance=int(threshold_values.get("raw_sloc_tolerance", defaults.raw_sloc_tolerance)),
        maintainability_ratchet=threshold_values["maintainability_ratchet"],
        maintainability_tolerance=threshold_values.get(
            "maintainability_tolerance", defaults.maintainability_tolerance
        ),
        halstead_volume_ratchet=threshold_values["halstead_volume_ratchet"],
        halstead_volume_ceiling=threshold_values["halstead_volume_ceiling"],
        halstead_volume_tolerance=threshold_values.get(
            "halstead_volume_tolerance", defaults.halstead_volume_tolerance
        ),
        halstead_difficulty_ratchet=threshold_values["halstead_difficulty_ratchet"],
        halstead_difficulty_ceiling=threshold_values["halstead_difficulty_ceiling"],
        halstead_difficulty_tolerance=threshold_values.get(
            "halstead_difficulty_tolerance", defaults.halstead_difficulty_tolerance
        ),
        halstead_effort_ratchet=threshold_values["halstead_effort_ratchet"],
        halstead_effort_ceiling=threshold_values["halstead_effort_ceiling"],
        halstead_effort_tolerance=threshold_values.get(
            "halstead_effort_tolerance", defaults.halstead_effort_tolerance
        ),
        halstead_bugs_ratchet=threshold_values["halstead_bugs_ratchet"],
        halstead_bugs_ceiling=threshold_values["halstead_bugs_ceiling"],
        halstead_bugs_tolerance=threshold_values.get(
            "halstead_bugs_tolerance", defaults.halstead_bugs_tolerance
        ),
    )
    cyclomatic_values = _number_dict(data.get("cyclomatic"), "cyclomatic")
    raw_values = _number_dict(data.get("raw_sloc"), "raw_sloc")
    halstead_value = data.get("halstead")
    if not isinstance(halstead_value, dict):
        raise ValueError("halstead must be an object")
    halstead = {
        field: _number_dict(halstead_value.get(field), f"halstead.{field}")
        for field in RATCHET_HALSTEAD_FIELDS
    }
    return Baseline(
        thresholds=thresholds,
        cyclomatic={key: int(value) for key, value in cyclomatic_values.items()},
        raw_sloc={key: int(value) for key, value in raw_values.items()},
        maintainability=_number_dict(data.get("maintainability"), "maintainability"),
        halstead=halstead,
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    mode.add_argument(
        "--strict", action="store_true", help="fail on metric regressions and hard ceiling violations"
    )
    mode.add_argument(
        "--ratchet",
        action="store_true",
    ...
    mode.add_argument("--update-baseline", action="store_true", help="replace the baseline with current debt")
    ...
    if args.update_baseline:
        try:
            thresholds = read_baseline(args.baseline).thresholds if args.baseline.exists() else Thresholds()
